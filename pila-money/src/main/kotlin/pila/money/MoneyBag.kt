...
data class MoneyBag(
    ...
    fun put(money: Money) = monies.getOrPut(money.currency) { mutableListOf() }.add(money)
    ...
    fun getAll(currency: Currency): List<Money> = monies[currency]?.toList().orEmpty()

    fun getAll(): List<Money> = monies.values.flatten()
}
